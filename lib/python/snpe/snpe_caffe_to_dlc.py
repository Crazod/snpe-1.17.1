    ...
    def check_bs_folding(self, layer):
        ...
        if scale_layer_seq is not None:
            if len(scale_layer_seq.bottom) > 1:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE_SCALE_FOLDING_TOO_MANY_INPUTS')(scale_layer_seq.name, str(scale_layer_seq.bottom)))
...
class RandomWeightProvider(object):
    ...
    def get_fc_weights(self, layer, input_depths, bias_term):
        ...
        for name in replaced_bottom:
            ...
            input_dims = self.model.get_buffer_dims(bufname)[1:] # Strip batch dimension for input size calculation
...
class CaffeSnapDnnConverter(object):
    ...
    def convert_caffe_new( self, spec ):
        ...
        if len(spec.input_shape) > 0 and len(spec.input_shape[0].dim) == 4:
            ...
            self.input_dim = map( int, [spec.input_shape[0].dim[0],
            ...
            self.input_dim = map(int, spec.input_shape[0].dim.insert(0, 1))
        elif len(spec.input_dim) == 4:
            # 4-D input_dim. Treat as (batch, depth, height, width)
            self.input_dim = map( int, [spec.input_dim[0],
                                        spec.input_dim[2],
                                        spec.input_dim[3],
                                        spec.input_dim[1]] )
            ...
            self.input_dim = map(int, spec.input_dim).insert(0,1)
        ...
        for index in range(1, len(spec.input)):
            ...
            if len(spec.input_shape[index].dim) == 4:
                # 4-D input_dim. Treat as (batch, depth, height, width)
                data_dims = map( int, [spec.input_shape[index].dim[0],
                ...
                data_dims = map( int, [1, 1, spec.input_shape[index].dim[0], spec.input_shape[index].dim[1]] )
            else:
    ...
    def add_input_layer(self, layer):
        ...
        self.input_dim = map( int, [input_param.shape[0].dim[0],
    ...
    def process_ssd_priorbox_layer(self, layer):
        ...
        # HWC format
        input_layer_width = input_dims[1]
        input_layer_height = input_dims[2]
