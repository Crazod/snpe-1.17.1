...
import snpe
from snpe_caffe2_to_dlc_utils import SNPEUtils
...
class BufferProxy(object):
    ...
    def dump(self):
        ...
        self._logger.debug(code_to_message.get_debugging_message('DEBUG_CAFFE2_INPUT_BUFFER_DUMP')(str(len(self._pending_input_buffer_proxy.keys()))))
        ...
        self._logger.debug(code_to_message.get_debugging_message('DEBUG_CAFFE2_OUTPUT_DUMP')(str(len(self._output_buffer_proxy.keys()))))
...
class PretrainedDataProvider(object):
    def __init__(self, init_net_path, ext_inputs):
        ...
        for data_op in net_params.op:
            if str(data_op.name) != '' or len(data_op.input) > 1:
                raise ValueError(
                    code_to_message.get_message('ERROR_CAFFE2_DATA_SINGLE_INSTANCE_EXPECTED')(str(data_op.name), data_op.input))
            if len(data_op.output) != 1:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_DATA_SINGLE_INSTANCE_EXPECTED')(str(data_op.name)))

            buffer_name = str(data_op.output[0])
            if buffer_name in self.weights_map.keys():
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_DUPLICATE_DATA_DETECTED')(buffer_name))
    ...
    def get_weights(self, name):
        ...
        if name not in self.weights_map.keys():
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_WEIGHT_NAME_NOT_IN_MAP')(str(name)))
        ...
        for arg in map_entry:
            if str(arg.name) == 'shape':
                dims = arg.ints
        if len(dims) == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_WEIGHT_SHAPE_NOT_IN_MAP')(str(name)))
        ...
        if len(weights) == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_WEIGHT_VALUES_NOT_IN_MAP')(str(name)))
    ...
    def get_batch_norm_weights(self, op, eps):
        ...
        if len(param_in) < 4:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_SPATIAL_BATCH_NORM_PARAMS_ORDER_ERR')(str(op.output[0])))
    ...
    def get_conv_weights(self, op):
        ...
        param_in = self.get_weight_inputs(op)
        if len(param_in) < 1:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_CONV_LAYER_INPUT_ERR')(str(op.output[0])))
    ...
    def get_deconv_weights(self, op):
        ...
        param_in = self.get_weight_inputs(op)
        if len(param_in) < 1:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_DECONV_LAYER_INPUT_ERR')(str(op.output[0])))
    ...
    def get_fc_weights(self, op, input_depths):
        ...
        param_in = self.get_weight_inputs(op)
        if len(param_in) < 2:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_OP_INPUT_WEIGHT_BIASES_ERR')(str(op.output[0])))
        weights = []
    ...
    def get_instance_norm_weights(self, op):
        ...
        param_in = self.get_weight_inputs(op)
        if len(param_in) < 2:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_NORMALIZATION_PARAMS_ORDER_ERR')(str(op.output[0])))
    ...
    def get_prelu_weights(self, op):
        param_in = self.get_weight_inputs(op)
        if len(param_in) < 1:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_PRELU_EXPECTED_SLOPE_PARAM_ERR')(str(op.output[0])))
...
class Caffe2SnapDnnConverter(object):
    ...
    def convert_op(self, op):
        if not str(op.type) in self.op_registry_.keys():
            self.logger.error(code_to_message.get_message('ERROR_CAFFE2_SNPE_OP_SUPPORT_ERR')(str(op.type)))
            sys.exit(1)

        try:
            id_ = self.op_registry_[str(op.type)](op)
        except Exception as e:
            self.logger.error(code_to_message.get_message('ERROR_CAFFE2_PROCESSING_OP_ERR')(str(op.type), str(e)))
            sys.exit(1)
    ...
    def convert(self,
        ...
        try:
            ...
            self.logger.error(code_to_message.get_message('ERROR_CAFFE2_PARSING_NETWORK_DEF')(net_def_path, str(e)))
            sys.exit(1)
        ...
        for data in self.reorder_list:
            if data not in self.external_inputs and data not in self.external_outputs:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_NOT_INPUT_OR_OUTPUT_FOR_REORDER')(data))
        ...
        if len(input_dims) == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_INPUT_DIMS_NOT_VALID'))

        for data_in in input_dims:
            if len(data_in) != 2:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_INPUT_DIMS_FORMAT_NOT_VALID'))
            dims = data_in[1].split(',')
            if len(dims) > 4:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_INPUT_DIMS_CHANNEL_FORMAT_NOT_VALID'))

            if data_in[0] not in map(str, self.net.external_input):
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_DATA_NOT_AN_EXTERNAL_DATA_INPUT')(data_in[0]))

            self.input_dims[data_in[0]] = map(int, dims)
    ...
    def process_net( self, net):
        ...
        # Some sanity checks
        if len(net.op) == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_NO_OPS_PRESENT_IN_CAFFE2_CONVERTER'))
        ...
        if len(self.input_dims) > 0 and not self.enable_preprocessing:
            ...
            for data_name, dims in self.input_dims.iteritems():

                input_type = self.input_type_map.get(data_name,"default")
                if len(dims) == 4: # Reorder dims from NCHW to NHWC
                    data_dims = map(int, [dims[0], dims[2], dims[3], dims[1]])
                else:
                    data_dims = map(int, dims)
        ...
        for op in net.op:
            ...
            if type_ == 'ImplodeBatch':
                ...
                if (str(prev.type) != 'RoIAlign'):
                    err = code_to_message.get_message('ERROR_CAFFE2_IMPLODE_BATCH_INPUT')
    ...
    def process_ops(self, net):
        ...
        for op in net.op:
            ...
            if "order" in args.keys() and args["order"] != "NCHW":
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_ONLY_NCHW_ORDER_SUPPORTED')(args["order"]))
...
#                code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_PROCESS_OUT_DIMS')(self.get_layer_name(op), op.type, str(self.get_output_dims(op))))
    ...
    def get_args(self, args):
        ...
        for arg in args:
            ...
            if arg_data is None:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_CANT_PROCESS_ARGS')(arg_name))
            if arg_name in parsed_args.keys():
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_DUPLICATE_ARG_FOUND')(arg_name))
    ...
    def check_args(self, args, supported_args):
        ...
        for arg in args.keys():
            if arg not in supported_args:
                bad_args.append(arg)
        if bad_args:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_UNSUPPORTED_ARGS_ERR')(str(bad_args)))
    ...
    def add_udl_layer(self, op):
        ...
        if blob.getSize() == 0:
            raise ValueError(code_to_message.get_message(ERROR_CAFFE2_UNEXPECTED_ZERO_BLOB_SIZE))
    ...
    def do_pre_layer_work(self, op):
        ...
        self.logger.debug(code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_PRE_WORK_OP')(self.get_layer_name(op)))
    ...
    def add_channel_shuffle_layer(self, op):
        ...
        if 'group' in args.keys():
           ...
           raise ValueError(code_to_message.get_message('ERROR_CAFFE2_CHANNEL_SHUFFLE_LAYER_MISSING_GROUPS_ARG')(str(op.type)))

        return self.model.add_channel_shuffle_layer( name=layer_name,
                                                   groups = snpe_groups,
                                                   shuffle_type = snpe.modeltools.CHANNEL_SHUFFLE_GROUPED,
                                                   input_name = str(self.get_input_name(op)),
                                                   output_name = str(self.get_output_name(op)))
    ...
    def add_concat_layer(self, op):
        ...
        args = self.get_args(op.arg)
        caffe2_axis = args['axis'] if 'axis' in args.keys() else 1
        snpe_axis = 2
        ...
        if len(input_dim) == 1:
            ...
            if not all(x == snpe_axes[0] for x in snpe_axes):
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_CONCAT_LAYER_AXIS_NOT_SUPPORTED')(layer.name, caffe2_axis))
    ...
    def add_conv_layer(self, op):
        ...
        pad_mode = snpe.modeltools.PADDING_ZERO
        ...
        if in_name in self.global_op_info and str(self.global_op_info[in_name].type) == "PadImage":
            ...
            if pad_args['mode'] == "reflect":
                self.logger.warn(code_to_message.get_warning_message('WARNING_CAFFE2_IGNORE_LOCAL_PADDING'))
                pad_mode = snpe.modeltools.PADDING_REFLECT
            else:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_UNSUPPORTED_PADDING_OP_FOR_CONV_LAYER')(in_name, str(pad_op.type)))
        ...
        if "dilation" in args.keys():
            dilation_x = args["dilation"]
            dilation_y = args["dilation"]
        elif "dilation_w" in args.keys() and "dilation_h" in args.keys():
            dilation_x = args["dilation_w"]
            dilation_y = args["dilation_h"]
        elif "dilations" in args.keys():
        ...
        if 'legacy_pad' in args.keys() and ( args['legacy_pad'] == 1 or args['legacy_pad'] == 2 ):
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_CONV_LEGACY_AND_DEFAULT_PADDING_SUPPORTED')(layer_name))

        # Caffe2 defaults to LegacyPadding::NOTSET (0)
        # It uses floor-based padding style
        padding_style = snpe.modeltools.PADDING_SIZE_EXPLICIT_FLOOR

        if 'legacy_pad' in args.keys() and args['legacy_pad'] == 3:
            padding_style = snpe.modeltools.PADDING_SIZE_EXPLICIT
    ...
    def add_elementwise_sum_layer(self, op):
        ...
        for index, dims in enumerate(in_dims):
            if in_dims[0] != dims:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_ADD_ONLY_SAME_INPUT_SHAPES_SUPPORTED_ERR')(layer_name))

        coeffs = []
        coeffs.extend( [1.0 for i in xrange(len(input_names))] )
    ...
    def add_generate_proposals_layer(self, op):
        ...
        if len(input_names) != 2:
            err = code_to_message.get_message('ERROR_CAFFE2_WRONG_NUMBER_OF_INPUTS')
            raise ValueError(err("Generate Proposals",layer_name,2,len(input_names)))

        output_names = self.get_output_names(op)
        if len(output_names) != 2:
            err = code_to_message.get_message('ERROR_CAFFE2_WRONG_NUMBER_OF_OUTPUTS')
    ...
    def add_pooling_layer(self, op):
        ...
        # Don't support LegacyPadding::VALID(1) and  LegacyPadding::SAME(2) yet
        if 'legacy_pad' in args.keys() and ( args['legacy_pad'] == 1 or args['legacy_pad'] == 2 ):
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_POOLING_LEGACY_AND_DEFAULT_PADDING_SUPPORTED')(layer_name))
        ...
        input_dim = self.get_input_dim(op)
        if 'global_pooling' in args.keys():
            ky = input_dim[1]
            kx = input_dim[2]
            stridex, stridey = 1, 1
            padx, pady = 0, 0
    ...
    def add_prelu_layer(self, op):
        ...
        if len(bias) == 1:
            ...
            for i in xrange(input_dims[0][3]-1):
                bias.append(bias[0])
    ...
    def add_lrn_layer(self, op):
        ...
        if not all (k in args.keys() for k in ("size","alpha","beta","bias")):
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_LRN_ARG_MISSING'))
    ...
    def add_reshape_layer(self, op):
        ...
        input_dims = map(int, self.model.get_buffer_dims(bufname))
        ...
        if typ == "Reshape":
           ...
           if len(output_dims) == 0:
               if not 'shape' in args:
                   raise ValueError(code_to_message.get_message('ERROR_CAFFE2_RESHAPE_OP_NO_INPUT_OR_ARG_SHAPE')(layer_name))
           ...
           raise ValueError(code_to_message.get_message('ERROR_CAFFE2_INVALID_OP_TYPE')(str(op.type)))
    ...
    def add_scaling_layer(self, op):
        ...
        if "width_scale" not in args.keys():
            width_scale = 0.0
        height_scale = 1.0
        if "height_scale" in args.keys():
        ...
        if "height_scale" not in args.keys():
            height_scale = 0.0
        ...
        return self.model.add_scaling_layer( name = layer_name,
                                             ...
                                             resize_mode = snpe.modeltools.RESIZE_NEAREST_NEIGHBOR,
    ...
    def add_slice_op(self, op):
        ...
        args = self.get_args(op.arg)
        if not all (k in args.keys() for k in ("start", "end")):
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_SLICE_OP_INDICIES_MISSING'))
        ...
        if len(start) != len(input_dim) or len(start) != len(end):
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_SLICE_OP_INPUT_DIM_MISMATCH'))

        if len(start) != 3 and len(start) != 4:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_SLICE_ONLY_SUPPORTED_FOR_3AND4_DIMS_DATA'))
    ...
    def add_split_op(self, op):
        ...
        if len(op.input) > 1:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_AXIS_ORDER_SPLIT_ARGS_ONLY_SUPPORTED'))
        ...
        slice_points = []
        if 'split' in args:
            slice_points = map(int, args['split'])
    ...
    def add_tile_layer(self, op):
        ...
        if len(inputs) == 1:
            caffe2_axis = args['axis'] if 'axis' in args.keys() else 0
            caffe2_tiles = args['tiles'] if 'tiles' in args.keys() else 1
        else:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_TILE_INPUTS_NOT_SUPPORTED'))
    ...
    def get_input_id(self, op):
        if len(op.input) == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_GET_INPUT_ID_INVALID_INPUT'))
    ...
    def get_output_name(self, op):
        if len(op.output) == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_GET_INPUT_ID_INVALID_OUTPUT'))
    ...
    def process_image_padding(self, op):
        ...
        args = self.get_args(op.arg)
        if 'mode' not in args or args['mode'] != "reflect":
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_REFLECT_PAD_MODE_ONLY_SUPPORTED')(args['mode']))
    ...
    def get_kps_op_params(self, op, args, pad_args = {}):
        ...
        pad = args['pad'] if 'pad' in args.keys() else 0
        stride = args['stride'] if 'stride' in args.keys() else 1
        kernel = args['kernel'] if 'kernel' in args.keys() else 0

        def get_pad(args):
            padx, pady = 0,0
            if "pad_t" in args.keys():
                pady = args['pad_t']
            if "pad_b" in args.keys():
                pady = args['pad_b']
            if "pad_l" in args.keys():
                padx = args['pad_l']
            if "pad_r" in args.keys():
                padx = args['pad_r']
            if "pad" in args.keys():
                padx, pady = pad,pad
            if "pads" in args.keys():
                pads = args["pads"]
        ...
        padx, pady = get_pad(pad_args)

        stridex, stridey = 1, 1
        if 'stride_h' in args.keys() or 'stride_w' in args.keys():
            stridex = args['stride_w']
            stridey = args['stride_h']
        elif 'strides' in args.keys():
        ...
        kx = 0
        ky = 0
        if 'kernels' in args.keys():
            kernels = args['kernels']
            if len(kernels) != 2:
                raise ValueError(code_to_message.get_message('ERROR_CAFFE2_ARG_KERNEL_LEN_NOT_EXPECTED')(len(kernels)))
            # Height comes first, then width
            ky = int(kernels[0])
            kx = int(kernels[1])
        elif 'kernel_h' in args.keys() or 'kernel_w' in args.keys():
            kx = args['kernel_w']
            ky = args['kernel_h']
        else:
    ...
    def setup_preprocessing(self, op):
        ...
        if len(net_dims) == 4:  # Reorder dims from NCHW to NHWC
            data_dims = map(int, [net_dims[0], net_dims[2], net_dims[3], net_dims[1]])
        elif len(net_dims) == 3:  # Reorder dims from CHW to HWC
            data_dims = map(int, [1, net_dims[1], net_dims[2], net_dims[0]])
        else:
            data_dims = map(int, dims)
        ...
        if  crop_size != 0 and (crop_size != original_height or crop_size != original_weight):
            if crop_size > original_height or crop_size > original_weight:
                errs = code_to_message.get_message('ERROR_CAFFE2_CROP_SIZE_LARGER_THAN_INPUT_DIMS')(crop_size, net_dims[0:2])
        ...
        if 'mean' in args.keys():
            mean_data = numpy.zeros(crop_dim, dtype=numpy.float32)
            mean_data[:] = float(args['mean'])

        if mean_data is not None:
            if mean_data.shape != tuple(crop_dim):
                errs = code_to_message.get_message('ERROR_CAFFE2_MEAN_DATA_WRONG_DIMS')(data_name, str(crop_dim), str(mean_data.shape))
                raise ValueError(errs)
