...
class VisitableGraph(object):
    ...
    def accept(self, visitor):
        ...
        for scope, ops in scopes.iteritems():
...
class GraphHelper(object):
    ...
    def dump(self):
        for n, s in self._tensor_shape_cache.items():
            print n, s
    ...
    def _evaluate_tensors_output_shape(self, tensors):
        ...
        for tensor, output in outputs_map.iteritems():
    ...
    def evaluate_tensors_output(self, tensors):
        ...
        for name, tensor in self._placeholders_stubs_map.iteritems():
        ...
        if len(requiring_evaluation) > 0:
            try:
                ...
                outputs = dict(zip(requiring_evaluation, outputs))
                for t, o in outputs.iteritems():
        ...
        for t in requiring_evaluation:
            try:
                ...
                outputs_map[t] = np.zeros((1,), dtype=np.float32)
        return outputs_map
    ...
    def filter_single_op_by_type(cls, operations, operation_type):
        ...
        if len(ops) > 1:
            raise OperationNotFoundError(
                code_to_message.get_message('ERROR_TF_MULTIPLE_NODES_FOUND')(operation_type))
        return ops[0]
    ...
    def get_op_input_tensors(cls, operations, input_types):
        ...
        if len(types) != len(input_types):
            raise TensorNotFoundError(
                code_to_message.get_message('ERROR_TF_INPUT_DOES_NOT_MATCH_COUNT')(operations.name, types, input_types))
        ...
        for i, t in enumerate(tensors):
            if types[i] == input_types[i] or input_types[i] == '?':
                ...
                raise TensorNotFoundError(
                    code_to_message.get_message('ERROR_TF_INPUT_DOES_NOT_MATCH_TYPES')(operations.name,
                                                                                       types,
                                                                                       input_types))
    ...
    def _evaluate_tensor_shapes(self, ops):
        ...
        try:
            ...
            logger.warning(code_to_message.get_message('ERROR_TF_FALLBACK_TO_ONDEMAND_EVALUATION'))
...
class OperationExecutionSorter(object):
    ...
    def sort(self, input_ops_names, output_ops_names):
        ...
        sorted_in_execution_order = sorted(self.ops_map.values(), cmp=lambda a, b: a.order - b.order)
        return [op.tf_op for op in sorted_in_execution_order]

    def _prepare_inputs_and_outputs(self, input_ops_names, output_ops_names):
        self.input_ops = []
        self.output_ops = []
        for op_wrapper in self.ops_map.values():
