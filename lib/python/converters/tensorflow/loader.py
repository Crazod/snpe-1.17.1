...
from converters.tensorflow.util import GraphPrinter
from converters.tensorflow.util import VisitableGraph
...
class ModelLoader(object):
    ...
    def load(self, graph_pb_or_meta_path, input_nodes_names, input_nodes_shapes, input_nodes_types, out_node_names,
        ...
        if len(input_nodes_names) != len(input_nodes_shapes):
            raise ConverterError(code_to_message.get_message('ERROR_TF_INPUT_NODE_SHAPE_DIMS_MISMATCH'))
        if input_nodes_types is not None:
            if len(input_nodes_names) != len(input_nodes_types):
                raise ConverterError(code_to_message.get_message('ERROR_TF_INPUT_TYPES_AND_NAMES_NOT_IN_PAIRS'))
        else:
        ...
        with session.graph.as_default():
            ...
            for name, shape, input_type in zip(input_nodes_names, input_nodes_shapes, input_nodes_types):
                ...
                try:
                    ...
                    input_shape = map(int, shape.split(','))
                ...
                if len(batched_shape) == 0:
                    try:
                        batched_shape = map(int, shape.split(','))
                    except ValueError:
                        raise ConverterError(code_to_message.get_message('ERROR_TF_INVALID_INPUT_DIMS')(shape))
    ...
    def __import_graph(cls, graph_path, session, out_nodes_names):
        ...
        if not os.path.exists(graph_path):
            raise ConverterError(code_to_message.get_message('ERROR_TF_GRAPH_FILE_DOES_NOT_EXIST')(graph_path))
        ...
        if len(graph_def.node) == 0:
            raise ConverterError(code_to_message.get_message('ERROR_TF_NODES_NOT_FOUND_IN_GRAPH'))
    ...
    def __import_from_meta_graph(cls, meta_graph_path, graph_path, out_nodes_names):
        ...
        with session.graph.as_default():
            try:
                ...
                raise ConverterError(code_to_message.get_message('ERROR_TF_CANNOT_IMPORT_GRAPH_FROM_META')(e.message))

            if saver is None:
                raise ConverterError(code_to_message.get_message('ERROR_TF_GRAPH_META_EMPTY'))
    ...
    def __assert_node_in_graph(cls, graph_def, node_name):
        if node_name not in [node.name for node in graph_def.node]:
            raise ConverterError(code_to_message.get_message('ERROR_TF_NODE_NOT_FOUND_IN_GRAPH')(node_name))
