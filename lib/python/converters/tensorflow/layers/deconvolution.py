...
from converters.tensorflow.layers.convolution import ConvolutionLayerBuilder
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for match in matches:
            ...
            if weights_tensor.op.type not in ['Identity', 'Const']:
                raise ConverterError(code_to_message.get_message('ERROR_TF_DECONV_CANT_FIND_WEIGHTS_NODE'))
            ...
            try:
                ...
                if biases.op.type not in ['Const', 'Identity']:
                    raise ConverterError(code_to_message.get_message('ERROR_TF_DECONV_CANT_FIND_BIAS_NODE'))
...
class DeConvolutionLayerBuilder(LayerBuilder, object):
    def build_layer(self, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        pad_y, pad_x, padding_strategy = ConvolutionLayerBuilder.calculate_padding_size(input_size=output_dims[-3:-1],
                                                                                        ...
                                                                                        dilation=[1, 1])
        if pad_y != pad_x:
            raise ConverterError(code_to_message.get_message('ERROR_TF_DECONV_NO_SUPPORT_RECT_PADDING'))
