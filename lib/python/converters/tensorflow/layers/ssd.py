...
from converters.code_to_message import get_message
from converters.tensorflow.common import LayerDescriptor, LayerResolver, LayerBuilder, ConverterError
from converters.tensorflow.layers.constant import ConstantLayerResolver
from converters.tensorflow.sequences import ssd
from converters.tensorflow.layers.reshape import ReshapeLayerResolver
...
class SSDDecoderResolver(LayerResolver, object):
    class Descriptor(LayerDescriptor):
        def __init__(self, name, nodes, scale_y, scale_x, scale_h, scale_w):
...
class SSDDecoderLayersBuilder(LayerBuilder):
    ...
    def build_layer(self, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        if len(anchor_input) != 1:
            raise ConverterError(get_message('ERROR_TF_SSD_ANCHOR_INPUT_MISSING'))
