    ...
    class Descriptor(LayerDescriptor):
        def __init__(self, name, operations, bn_mul_op, pre_calculated=False, *args, **kwargs):
            ...
            if pre_calculated:
                ...
                self.biases = (-1 * scaled_mean) + beta

    def resolve_layer(self, graph_matcher, graph_helper):
        raise ConverterError(code_to_message.get_message('ERROR_TF_GENERAL_ABSTRACT_CLASS_MUST_BE_INHERITED'))
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for match in matches:
            ...
            if variance_op.type not in ['Identity', 'Const']:
                raise ConverterError(code_to_message.get_message('ERROR_TF_BATCHNORM_RESOLVE_VARIANCE'))
            variance = graph_helper.evaluate_tensor_output(variance_op.outputs[0])

            if epsilon_op.type not in ['Identity', 'Const']:
                raise ConverterError(code_to_message.get_message('ERROR_TF_BATCHNORM_RESOLVE_EPSILON'))
            ...
            if scale_op.type not in ['Identity', 'Const', 'Fill']:
                raise ConverterError(code_to_message.get_message('ERROR_TF_BATCHNORM_RESOLVE_SCALE'))
            scale = graph_helper.evaluate_tensor_output(scale_op.outputs[0])

            mean_op = match['mean']
            if mean_op.type not in ['Identity', 'Const']:
                raise ConverterError(code_to_message.get_message('ERROR_TF_BATCHNORM_RESOLVE_MEAN'))
            mean = graph_helper.evaluate_tensor_output(mean_op.outputs[0])

            beta_op = match['beta']
            if beta_op.type not in ['Identity', 'Const']:
                raise ConverterError(code_to_message.get_message('ERROR_TF_BATCHNORM_RESOLVE_BETA'))
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for match in matches:
            ...
            if len(parameter_tensors) < 4:
                raise ConverterError(code_to_message.get_message('ERROR_TF_BATCHNORM_GLOBALNORMALIZATION_INPUT'))
